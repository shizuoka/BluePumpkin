...
//    }
    ...
    public void updatePize(ActionEvent event) {
        //System.out.println(evId);
        int numberPrize;
        
        if (prizeEdit.getNumberOfPrize() == null || prizeEdit.getNumberOfPrize().toString().equals("")) {
            numberPrize = 1;
        } else {
            numberPrize = prizeEdit.getNumberOfPrize();
        }
        
//        boolean result = prizesFacade.upatePrize(prizeEdit.getDescription(),prizeEdit.getNumberOfPrize(),prizeEdit.getPrizeID());
//        if (result) {
//            FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "SUCCESSFULL", "Add New successfull");
//            FacesContext.getCurrentInstance().addMessage(null, message);
//        } else {
